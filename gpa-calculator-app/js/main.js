/**
 * =================================
 * GPA Calculator - Main Logic (GitHub-Compatible Version)
 * =================================
 * This script handles all the application logic. It no longer fetches
 * components, as they are now all present in index.html.
 * It focuses on:
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function initializeApp() {
        ...
        if (state.modules && state.modules.length > 0) {
            const list = get('#modules-list');
            if(list) { // Check if the list element exists
                list.innerHTML = '';
                // This function is defined inside setupEventListeners, so we need to either move it out or call setupEventListeners first.
                // Let's call setupEventListeners first. This is a dependency issue.
            }
        }
        setupEventListeners(); // Call this to define addModuleToList

        // Now that addModuleToList is defined, we can populate the list
        if (state.modules && state.modules.length > 0) {
            const list = get('#modules-list');
            list.innerHTML = '';
            // We need to access addModuleToList, which is defined inside setupEventListeners.
            // Let's restructure slightly.
            const { addModuleToList } = setupEventListeners();
    ...
    /**
     * This is a refactored version of the initialization logic to resolve scope issues.
     */
    function finalInit() {
        const get = (selector) => document.querySelector(selector);
        
        // All other functions from initializeApp go here...
        // ... (state, translations, UI updaters, etc.)
        
        // The core issue is that `addModuleToList` is defined inside `setupEventListeners`.
        // Let's define it in a higher scope.
        
        let T_GLOBAL; // To hold translations globally within this scope.
        ...
        const updateModuleInList = (item, data) => {
            ...
            item.querySelector('.kebab-menu').addEventListener('click', (e) => {
                e.stopPropagation();
                const menu = item.querySelector('.module-options-menu');
                document.querySelectorAll('.module-options-menu').forEach(m => { if(m !== menu) m.style.display = 'none'; });
                menu.style.display = menu.style.display === 'block' ? 'none' : 'block';
            });
            item.querySelector('.delete-btn').addEventListener('click', () => {
                item.remove();
                ...
                // We need access to saveState
                // This tight coupling is why larger apps use frameworks or classes.
                // Let's just go with the simple, reliable, full-code approach.
            });
            item.querySelector('.edit-btn').addEventListener('click', () => {
                 get('#modal-title').textContent = T.edit_module_title;
                 get('#module-modal').dataset.mode = 'edit';
                 get('#module-modal').dataset.editId = item.dataset.id;
                 // Need resetModalForm and openModal
                 // Okay, the original full script was better structured, let's just remove the fetch part.
            });
        };
    }
    
    // The most straightforward way is to put the entire original script back,
    // but remove the `loadComponents` function and call `initializeApp` directly.
        ...
        const setupEventListeners = () => {
            ...
            get('#save-statement-btn').addEventListener('click', () => {
                ...
                html2canvas(statementElement, {
                    ...
                    if(watermarkDark) watermarkDark.style.display = 'none';
                    const link = document.createElement('a');
                    link.download = 'Statement_of_Marks.png';
                    link.href = canvas.toDataURL('image/png');
                    link.click();
                }).catch(err => {
                    console.error("Oops, something went wrong!", err);
            ...
            const formatGradeInput = (e) => {
                ...
                let value = input.value;
                let sanitizedValue = value.replace(/[^0-9.,]/g, '').replace(/,/g, '.').replace(/(\..*)\./g, '$1');
                if (parseFloat(sanitizedValue) > 20) { sanitizedValue = '20.00'; }
                let parts = sanitizedValue.split('.');
                if (parts[1] && parts[1].length > 2) {
                    sanitizedValue = parts[0] + '.' + parts[1].substring(0, 2);
                }
            ...
            getAll('input[data-is-grade-input="true"]').forEach(input => input.addEventListener('input', formatGradeInput));
            ...
            get('#reset-annual-btn').addEventListener('click', () => {
                const btn = get('#reset-annual-btn');
                if (get('#s1-avg').value.trim() !== '' || get('#s2-avg').value.trim() !== '' || get('#s1-credits').value.trim() !== '' || get('#s2-credits').value.trim() !== '') {
                    showConfirmationModal('confirm_delete_all_modules', clearAnnualInputs);
                } else {
                    btn.classList.add('shake');
                    setTimeout(() => btn.classList.remove('shake'), 500);
            ...
            getAll('#module-modal .toggle-switch').forEach(toggle => {
                toggle.addEventListener('click', () => {
                    ...
                    toggle.classList.toggle('active', isActive);
                    const controlsInput = get(`#${toggle.dataset.controls}`);
                    if(isActive) { controlsInput.focus(); }
                    const isTd = toggle.dataset.controls === 'td-input';
                    const isTp = toggle.dataset.controls === 'tp-input';
                    if (isActive && isTd) {
                        const tpGroup = get('[data-controls="tp-input"]').closest('.grade-input-group');
            ...
            get('#save-module-btn').addEventListener('click', () => {
                ...
                const td_tp_weight = 1 - state.examWeight;
                if (examActive && (tdActive || tpActive)) {
                    finalGrade = (exam * state.examWeight) + ((tdActive ? td : tp) * td_tp_weight);
                } else if (examActive) { finalGrade = exam; }
                else if (tdActive) { finalGrade = td; }
                else if (tpActive) { finalGrade = tp; }
            ...
            get('#show-semester-result-btn').addEventListener('click', () => {
                ...
                modules.forEach(item => {
                    ...
                    totalPoints += grade * coeff;
                    totalCoeffs += coeff;
                    if (grade >= 10) { earnedCredits += credits; }
                });
                const average = totalCoeffs > 0 ? totalPoints / totalCoeffs : 0;
                const finalCredits = average >= 10 ? 30 : earnedCredits;
                showResultModal({ titleKey: 'semester_result_title', average, credits: finalCredits, remarkKey: getRemarkKey(average) });
            });
            ...
            get('#show-annual-result-btn').addEventListener('click', () => {
                ...
                showResultModal({ isAnnual: true, titleKey: 'annual_result_title', average: annual_avg, credits: total_credits, status });
            });
        };
